// Extract
// <binary> x 'fileName.extension' -p<password> -o'Directory Name'
...
interface CompressOptions {
  ...
  compression_method?:
    ...
    | TAR_Method
    | WIM_Method;
  compression_level?: Level | ZIP_Level | SevenZ_Level | GZ_Level | XZ_Level | BZIP2_Level | TAR_Level | WIM_Level;
  encrypt?: boolean;
...
export class SevenZip extends EventEmitter {
  ...
  private get_stats(str: string) {
    ...
    str.split('\n').forEach((item) => {
      ...
      if (x.indexOf('Folders') != -1) {
        info.folders = Number.parseInt(x.substring(x.indexOf(':') + 1, x.length));
      }
      ...
      if (x.indexOf('Compressed') != -1) {
        info.compressed = Number.parseInt(x.substring(x.indexOf(':') + 1, x.length));
      }
    });
  ...
  private async compress(
    ...
    //<binary> x 'fileName.extension' -p<password> -o'Directory Name'
    // console.log(`Command :: ${this.binary} ${args}`);
    ...
    proc.stderr.on('data', (data: Buffer) => {
      ...
      this.emit('onFinish', { err: data.toString(), info: {} });
    });

    proc.on('exit', (code: number) => {
      this.emit('onProgress', 100);
      this.emit('onFinish', { err: code, info: {} });
    });
  }
  ...
  create = {
    zip: async (
      ...
      await this.compress(filePath, includeDir, {
        archive_type: ArchiveType.ZIP,
        ...options,
        encrypt_name: false,
      });
    ...
    tar: async (filePath: string, includeDir: string) => {
      await this.compress(filePath, includeDir, {
    ...
    wim: async (filePath: string, includeDir: string) => {
      await this.compress(filePath, includeDir, {
  ...
  async extract(filePath: string, outputDir: string, options: ExtractOptions = { password: '' }) {
    // Preparation of Command
  ...
  async hash(filePath: string, hasher: Hasher = Hasher.CRC32) {
    // Preparation of Command
    let command = `"${filePath}" ${hasher}`;

    //<binary> h 'fileName.extension' -scrc<hash>'
    // console.log(`Command :: ${command}`);
    ...
    proc.stdout.on('data', (data: Buffer) => {
      ...
      let hash = this.get_hash(str, hasher === Hasher.ALL ? true : false);
      if (hash) {
        this.emit('onProgress', 100);
        this.emit('onFinish', { err: 0, payload: hash });
      }
    });

    proc.stderr.on('data', (data: Buffer) => {
      console.log(`stderr: ${data}`);
      this.emit('onFinish', { err: data.toString(), payload: {} });
    });
  }

  async hash_ret(filePath: string, hasher: Hasher = Hasher.CRC32) {
    // Preparation of Command
    ...
    try {
      let ret = '';
      await exec(command, (err: Error, data: any) => {
        if (err) throw err;
        ret = data;
        console.log('calling');
      });
